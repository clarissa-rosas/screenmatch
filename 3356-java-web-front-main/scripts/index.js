...
geraSeries();
function geraSeries() {
    const urls = ['/series', '/series/top5', '/series/lancamentos'];

    // Faz todas as solicitações em paralelo
    Promise.all(urls.map(url => getDados(url)))
        .then(data => {
            criarListaFilmes(elementos.series, data[0].slice(0, 5));
            criarListaFilmes(elementos.top5, data[1]);
            criarListaFilmes(elementos.lancamentos, data[2]);
